...
export class PackServiceImpl implements PackService {
    constructor(private readonly logger: LoggerService, private readonly packRepo: PackRepository) {}
    ...
    async createPack(pack: NewPack, ownerId: string, pets: CreatePetInput[]): Promise<PackId> {
        ...
        const result = await this.packRepo.createPack(pack, ownerId, pets);
    ...
    async createPet(packId: string, pet: CreatePetInput): Promise<PetId> {
        ...
        const result = await this.packRepo.createPet(packId, pet);
    ...
    async getPack(packId: string): Promise<Pack | null> {
        ...
        const pack = await this.packRepo.getPack(packId);
    ...
    async getPackPets(packId: string): Promise<Pet[]> {
        ...
        const pets = await this.packRepo.getPackPets(packId);
    ...
    async getUserPacks(userId: string): Promise<Pack[]> {
        ...
        const packs = await this.packRepo.getUserPacks(userId);
